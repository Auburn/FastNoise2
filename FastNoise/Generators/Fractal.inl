...
{
    float32v sum = FS_CLASS( Modifier )<FS>::mSource->Gen( seed, pos... );
    ...
    for ( int i = 1; i < mOctaves; i++ )
    {
        seed -= int32v( -1 );
        amp *= float32v( mGain );
        sum += FS_CLASS( Modifier )<FS>::mSource->Gen( seed, (pos *= float32v( mLacunarity ))... ) * amp;
    }
...
{
    float32v sum = FS_Abs_f32( FS_CLASS( Modifier )<FS>::mSource->Gen( seed, pos... ) ) * float32v( 2 ) - float32v( 1 );
    float32v amp = float32v( 1 );
    ...
    for ( int i = 1; i < mOctaves; i++ )
    {
        seed -= int32v( -1 );
        amp *= float32v( mGain );
        sum += (FS_Abs_f32( FS_CLASS( Modifier )<FS>::mSource->Gen( seed, (pos *= float32v( mLacunarity ))... ) ) * float32v( 2 ) - float32v( 1 )) * amp;
    }
...
{
    float32v sum = float32v( 1 ) - FS_Abs_f32( FS_CLASS( Modifier )<FS>::mSource->Gen( seed, pos... ) );
    float32v amp = float32v( 1 );
    ...
    for ( int i = 1; i < mOctaves; i++ )
    {
        seed -= int32v( -1 );
        amp *= float32v( mGain );
        sum -= (float32v( 1 ) - FS_Abs_f32( FS_CLASS( Modifier )<FS>::mSource->Gen( seed, (pos *= float32v( mLacunarity ))... ) )) * amp;
    }
...
FS_CLASS( FastNoise::FractalRidgedMulti )<F, S>::GenT( int32v seed, P... pos )
{
    ...
    float32v sum = offset - FS_Abs_f32( FS_CLASS( Modifier )<FS>::mSource->Gen( seed, pos... ) );
    //sum *= sum;
    float32v amp = sum;
    ...
    for ( int i = 1; i < mOctaves; i++ )
    {
        ...
        seed -= int32v( -1 );
        float32v value = (offset - FS_Abs_f32( FS_CLASS( Modifier )<FS>::mSource->Gen( seed, (pos *= float32v( mLacunarity ))... ) ));
