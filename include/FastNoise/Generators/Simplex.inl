...
class FastSIMD::DispatchClass<FastNoise::Simplex, SIMD> final : public virtual FastNoise::Simplex, public FastSIMD::DispatchClass<FastNoise::VariableRange<ScalableGenerator>, SIMD>
{
    ...
    float32v FS_VECTORCALL Gen( int32v seed, float32v x, float32v y, float32v z, float32v w ) const
    {
        ...
        float32v gradientRampValue4 = GetGradientDotSimplex( HashPrimes( seed, xPrimedBase + int32v( Primes::X ), yPrimedBase + int32v( Primes::Y ), zPrimedBase + int32v( Primes::Z ), wPrimedBase + int32v( Primes::W ) ), dx4, dy4, dz4, dw4 );
...
class FastSIMD::DispatchClass<FastNoise::SuperSimplex, SIMD> final : public virtual FastNoise::SuperSimplex, public FastSIMD::DispatchClass<FastNoise::VariableRange<ScalableGenerator>, SIMD>
{
    float32v FS_VECTORCALL Gen( int32v seed, float32v x, float32v y ) const
    {
        ...
        float32v falloffBaseBase, value;

        // Vertex <0, 0>
        {
            int32v hash = HashPrimes( seed, xPrimedBase, yPrimedBase );
            float32v gradientRampValue = GetGradientDotSimplex( hash, dxBase, dyBase );
            falloffBaseBase = FS::FNMulAdd( dxBase, dxBase, FS::FNMulAdd( dyBase, dyBase, float32v( 2.0f / 3.0f ) ) );
            float32v falloff = falloffBaseBase; falloff *= falloff; falloff *= falloff;
        ...
        // Vertex <1, 1>
        {
            ...
            float32v gradientRampValue = GetGradientDotSimplex( hash, dxBase - float32v( 2 * G2 + 1 ), dyBase - float32v( 2 * G2 + 1 ) );
            float32v falloff = FS::FMulAdd( float32v( 2 * ( 1 + 2 * G2 ) * ( 1 / G2 + 2 ) ), unskewDelta, falloffBaseBase + float32v( -2 * ( 1 + 2 * G2 ) * ( 1 + 2 * G2 ) ) );
        ...
        // Vertex <1, 0> or <-1, 0> or <1, 2>
        {
            ...
            float32v dx = dxBase - FS::Select( forwardX, float32v( 1 + 2 * G2 ), float32v( -1 ) );
            float32v dy = FS::MaskedSub( forwardX, dyBase, float32v( 2 * G2 ) );
            float32v gradientRampValue = GetGradientDotSimplex( hash, dx, dy );
            float32v falloff = FS::Max( FS::FNMulAdd( dx, dx, FS::FNMulAdd( dy, dy, float32v( 2.0f / 3.0f ) ) ), float32v( 0 ) );
            falloff *= falloff; falloff *= falloff;
            value = FS::FMulAdd( falloff, gradientRampValue, value );
        }
        ...
        // Vertex <0, 1> or <0, -1> or <2, 1>
        {
            ...
            float32v dx = FS::MaskedSub( forwardY, dxBase, float32v( 2 * G2 ) );
            float32v dy = dyBase - FS::Select( forwardY, float32v( 1 + 2 * G2 ), float32v( -1 ) );
            float32v gradientRampValue = GetGradientDotSimplex( hash, dx, dy );
            float32v falloff = FS::Max( FS::FNMulAdd( dx, dx, FS::FNMulAdd( dy, dy, float32v( 2.0f / 3.0f ) ) ), float32v( 0 ) );
            falloff *= falloff; falloff *= falloff;
            value = FS::FMulAdd( falloff, gradientRampValue, value );
